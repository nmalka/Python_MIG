...
def netmask2int(mask):    
    """
    Convert Netmask format from 255.255.255.0 format to 24 format
    Added by Itzik Ben-Itzhak
    """
    binstr = ''
    for n in mask.split("."):
        n = int(n)
        if n < 0:
            raise ValueError, \
                "Failed to convert netmask from \".\" to integer(bin), netmask must be a positive integer"
        if n == 0: continue
        while n > 0:
